...
# Define path for input and output files - using relative paths
$inputFileName = "report.docx"
$outputFileName = "HYT_Ditch_Data.csv"

# Convert to absolute paths based on the script's location
$scriptPath = Split-Path -Parent -Path $MyInvocation.MyCommand.Definition
$inputPath = Join-Path -Path $scriptPath -ChildPath $inputFileName
...
if (-not (Test-Path -Path $inputPath)) {
    ...
    Write-Host "1. The file '$inputFileName' exists in the same folder as this script" -ForegroundColor Yellow
    Write-Host "2. The filename spelling matches exactly (including case)" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "You can also place the Word document in this location:" -ForegroundColor Yellow
...
$csvHeader | Out-File -FilePath $outputPath -Encoding ascii
...
$content = Get-WordContent -FilePath $inputPath
...
foreach ($section in $sections) {
    ...
    $csvLine | Out-File -FilePath $outputPath -Encoding ascii -Append
...
# Keep the PowerShell window open until the user presses a key
Write-Host "`nPress any key to exit..." -ForegroundColor Cyan
$null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")